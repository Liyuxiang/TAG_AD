...
int dwt_configure(dwt_config_t *config, uint8 use_otpconfigvalues)
{
    ...
    dw1000local.sysCFGreg |= (((uint32)config->phrMode) << 16) ;
...
//    configure PLL2/RF PLL block CFG
    ...
    {
        uint32 x = (((uint32)config->txPreambLength) | config->prf)  <<  16;
...
int dwt_writetxfctrl(uint16 txFrameLength, uint16 txBufferOffset)
{
    ...
    uint32 reg32 = dw1000local.txFCTRL | txFrameLength | (((uint32)txBufferOffset) << 22);
