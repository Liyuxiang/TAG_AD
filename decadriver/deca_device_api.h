...
#define DWT_LOADUCODE    0x800                      // ONW_LLDE - on wakeup load the LDE ucode
#define DWT_PRESRV_SLEEP 0x100                      // PRES_SLEEP - on wakeup preserve sleep bit
#define DWT_LOADOPSET    0x080						// ONW_L64P - on wakeup load operating parameter set for 64 PSR 
#define DWT_CONFIG       0x040						// ONW_LDC - on wakeup restore (load) the saved configurations (from AON array into HIF)
#define DWT_TANDV        0x001                      // ONW_RADC - on wakeup run ADC to sample temperature and voltage sensor values
